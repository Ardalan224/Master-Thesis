# -*- coding: utf-8 -*-
"""ItemKNN(Updated_nDCG)_LensKit_Gowalla_UserBasedSplit+10_Core_Pruning.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1NMNyurhfiwmDYrB9OAjJMqLdwogSDpZX
"""
...
# Group the dataset by 'user' and count the number of interactions per user
user_interaction_counts = ratings.groupby('user').size()

# Calculate the average number of interactions per user
average_interactions = user_interaction_counts.mean()

print(f"Average number of interactions per user before 10-core pruning: {average_interactions:.2f}")

import pandas as pd
import matplotlib.pyplot as plt

# Group the dataset by 'user' and count the number of interactions per user
user_interaction_counts = ratings.groupby('item').size()

# Calculate the average number of interactions per user
average_interactions = user_interaction_counts.mean()

print(f"Average number of interactions per item before 10-core pruning: {average_interactions:.2f}")
...
print(f"Average number of interactions per item: {average_interactions:.2f}")
...
# Create a bar plot for the distribution of ratings by percentage
plt.figure(figsize=(8, 6))
plt.bar(rating_percentages.index, rating_percentages.values, color='skyblue')
plt.title('Distribution of Ratings by Percentage, Amazon Toys & Games')
plt.xlabel('Rating')
plt.ylabel('Percentage (%)')
plt.ylim(0, 100)  # The y-axis should range from 0 to 100 since it's a percentage
plt.xticks(rating_percentages.index)  # Ensure all rating labels are shown
plt.show()
